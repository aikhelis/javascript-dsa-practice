...
class DoublyLinkedList {
    ...
    create pointers: let left = head, right = tail;
    loop through the list simaltenuously from head and from tail until meet in the middle 
        while (left !== right && left.pev !== right.next)
             return false if values of left and right are not equal
             left = left.next;
             right = right.prev;
    return true
    */
    isPalindrome() {
        ...
        let left = this.head, right = this.tail;
        while (left !== right && left.pev !== right.next) {
